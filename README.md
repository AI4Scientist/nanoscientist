...
```mermaid
flowchart TD
    I([Initializer\nzero LLM calls]) -->|research| PL[PlanInitialExecutor\n1 LLM call · drafts plan]

    PL -->|execute| EX

    subgraph EXEC["  Plan Loop  "]
        EX[PlanDrivenExecutor\npop step → run skill or write section]
        EX -->|"next step"| EX
    end

    EX -->|"budget low · review"| RV

    subgraph REVIEW["  Review Cycle  "]
        RV[ReviewExecutor\npeer-review · append revision steps]
    end

    RV -->|"revisions added · execute"| EX
    RV -->|"accepted · compile"| CT
